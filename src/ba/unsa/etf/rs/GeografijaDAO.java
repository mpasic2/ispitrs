...
public class GeografijaDAO {
    ...
    private GeografijaDAO() {
        ...
        try {
            ...
            //DODAN UPIT ZA PROMJENU DRZAVE
            promijeniDrzavuUpit = conn.prepareStatement("UPDATE drzava SET naziv=?, glavni_grad=? WHERE id=?");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void izmijeniDrzavu(Drzava grad) {
        try {
            promijeniDrzavuUpit.setString(1, grad.getNaziv());
            promijeniDrzavuUpit.setInt(2, grad.getGlavniGrad().getId());
            //promijeniDrzavuUpit.setInt(3, grad.getDrzava().getId());
            promijeniDrzavuUpit.setInt(3, grad.getId());
